...
export const AddIpad = async (req: Request, res: Response): Promise <Response |void> => {
  let filename = "";
  try {
    ...
    const newIpab = await sql`
      insert into itabs (tablet_name, product_picture, tablet_ram, tablet_rom, tablet_screen_type, tablet_curved_screen, tablet_screen_size, tablet_screen_resolution, tablet_chip, tablet_main_camera, tablet_selfie, tablet_physical_sim, tablet_esim, tablet_ai_integrated, tablet_touch_id, tablet_face_id, tablet_apple_pay, tablet_apple_pencil, tablet_magic_keyboard, tablet_battery, tablet_wired_charging, tablet_wireless_charging, tablet_mark, tablet_type, tablet_model, tablet_price, tablet_discount, tablet_price_discount, tablet_available_colors)
...
export const GetIpadById = async (req: Request, res: Response): Promise <Response | void> => {
  try {
    const { id } = req.params;
    ...
    const phone = await sql`select * from itabs where tablet_id = ${receivedId}`;
    if (phone.length === 0) {
