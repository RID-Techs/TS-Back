...
import { Galaxy_Tablets } from "../../../Types/all_types.js";
...
export const AddGalaxyTab = async (req: Request, res: Response): Promise <Response |void> => {
  ...
  try {
    ...
    const existingProduct = await sql`select tablet_id from galaxy_tabs where tablet_name = ${tablet_name}`;
    ...
    return res.status(201).json({ SuccessMsg: "Nouveau Samsung Galaxy Tab ajouté !"});
  } catch (error) {
...
export const GetAllGalaxyTabs = async (req: Request, res: Response): Promise <Response | void> => {
  try {
    const allGalaxyTabs = await sql`select * from galaxy_tabs`;
...
export const GetGalaxyTabById = async (req: Request, res: Response): Promise <Response | void> => {
  try {
    const { id } = req.params;
    const receivedId = id;
    if (!receivedId) {
