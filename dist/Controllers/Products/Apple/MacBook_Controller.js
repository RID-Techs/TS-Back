...
export const AddMacBook = async (req, res) => {
    let filename = "";
    try {
        ...
        if (!computer_name || !computer_ram || !computer_rom || !computer_screen_type || !computer_screen_size || !computer_screen_resolution || !computer_touch_screen || !computer_chip || !computer_core_number || !computer_integrated_graphics || !computer_gpu_core_number || !computer_neural_engine_core || !computer_card_slot || !computer_hdmi_port || !computer_jack_port || !computer_magsafe_port || !computer_usbc_port || !computer_usbc_port_number || !computer_battery || !computer_battery_autonomy || !computer_os || !computer_camera || !computer_ai_integrated || !computer_touch_id || !computer_mark || !computer_type || !computer_model || !computer_price) {
      ...
      insert into imacbook (computer_name, product_picture, computer_ram, computer_rom, computer_screen_type, computer_screen_size, computer_screen_resolution, computer_touch_screen, computer_chip, computer_core_number, computer_integrated_graphics, computer_gpu_core_number, computer_neural_engine_core, computer_card_slot, computer_hdmi_port, computer_jack_port, computer_magsafe_port, computer_usbc_port, computer_usbc_port_number, computer_battery, computer_battery_autonomy, computer_os, computer_camera, computer_ai_integrated, computer_touch_id, computer_mark, computer_type, computer_model, computer_price, computer_discount, computer_price_discount)
      values (${computer_name}, ${pictureUrl}, ${computer_ram}, ${computer_rom}, ${computer_screen_type}, ${Number(computer_screen_size)}, ${computer_screen_resolution}, ${computer_touch_screen}, ${computer_chip}, ${Number(computer_core_number)}, ${computer_integrated_graphics}, ${Number(computer_gpu_core_number)}, ${Number(computer_neural_engine_core)}, ${computer_card_slot}, ${computer_hdmi_port}, ${computer_jack_port}, ${computer_magsafe_port}, ${computer_usbc_port}, ${Number(computer_usbc_port_number)}, ${computer_battery}, ${Number(computer_battery_autonomy)}, ${computer_os}, ${Number(computer_camera)}, ${computer_ai_integrated}, ${computer_touch_id}, ${computer_mark}, ${computer_type}, ${computer_model}, ${Number(computer_price)}, ${computer_discount}, ${discountPrice})
      returning *
    `;
...
export const GetAllMacBooks = async (req, res) => {
    try {
        ...
        return res.status(200).json(allMacBooks);
    }
    catch (error) {
...
export const GetMacBookById = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const item = await sql `select * from imacbook where computer_id = ${receivedId}`;
        if (item.length === 0) {
